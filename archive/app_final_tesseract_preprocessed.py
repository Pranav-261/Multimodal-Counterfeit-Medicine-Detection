# -*- coding: utf-8 -*-
"""
app_final_tesseract_preprocessed.py

Flask app using Tesseract OCR with enhanced image preprocessing.
Loads the model trained with text augmentation. Runs on CPU.
"""
...
# !!! USER ACTION: Ensure this points to the model trained WITH augmentation !!!
MODEL_FILENAME = r"mm_cmds_model_finetune_robust_final.h5" # Ensure this is the correct model file
...
THRESHOLD_BLOCK_SIZE = 11 # For adaptive thresholding - must be odd
THRESHOLD_C = 5       # For adaptive thresholding - constant subtracted
# =============================================================================
...
if not os.path.exists(MODEL_PATH): logging.error(f"FATAL: Model file not found at {MODEL_PATH}")
    ...
    try:
        model = keras.models.load_model(MODEL_PATH) # Load H5 model
...
def preprocess_image_for_ocr_enhanced(image_path):
    """Applies more robust preprocessing for Tesseract OCR."""
    try:
        img = cv2.imread(image_path)
        if img is None: logging.warning(f"Failed OCR prep: load {image_path}"); return None
        ...
        # Apply Adaptive Thresholding (often better than simple thresholding)
        # Adjust block_size and C based on experiments if needed
        binary_img = cv2.adaptiveThreshold(contrast_enhanced, 255,
                                           cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                           cv2.THRESH_BINARY_INV, # Invert threshold - black text on white bg
                                           THRESHOLD_BLOCK_SIZE, THRESHOLD_C)

        # Optional: Noise reduction (try after thresholding)
        # binary_img = cv2.medianBlur(binary_img, 3) # Kernel size 3

        # Convert back to PIL Image format needed by pytesseract
        img_pil = Image.fromarray(binary_img)
        logging.info("Applied Grayscale, CLAHE, Adaptive Thresholding for OCR.")
        return img_pil

    except Exception as e:
        logging.error(f"Error during enhanced image preprocessing for OCR {image_path}: {e}", exc_info=True)
        return None
...
def predict():
    """Handles upload, preprocessing, targeted OCR, prediction."""
    ...
    if file and allowed_file(file.filename):
        ...
        except Exception as e: logging.error(f"Error saving: {e}", exc_info=True); return render_template('result.html', error_message="Save Error.")
        ...
        img_pil_for_ocr = preprocess_image_for_ocr_enhanced(image_path) # Use enhanced preprocessing
        ocr_text = ""
        if img_pil_for_ocr is not None:
            logging.info("Performing OCR using Tesseract...")
            try:
                # Experiment with different PSM modes here if needed (e.g., --psm 3 or --psm 11)
                tesseract_config = '--psm 6'
                ocr_text = pytesseract.image_to_string(img_pil_for_ocr, lang='eng', config=tesseract_config)
                ocr_text = ocr_text.replace('\n', ' ').replace('\f', '').strip() # Clean common artifacts
                logging.info(f"Tesseract OCR Result (Config: {tesseract_config}): '{ocr_text}'")
            except ImportError: logging.error("pytesseract/Tesseract error."); return render_template('result.html', error_message="OCR Setup Error.", image_filename=filename)
            except Exception as e: logging.warning(f"Error during Tesseract OCR: {e}", exc_info=True)
...
if __name__ == '__main__':
     if model is None or not char_map: print("\nERROR: Model/Char Map failed load.")
        ...
        print("\n--- Starting Flask Server (Enhanced Tesseract OCR) ---"); print(f"Model: {MODEL_FILENAME}"); print(f"Character Map Size: {len(char_map)}")
        print(f"Upload Folder: {app.config['UPLOAD_FOLDER']}"); print("Navigate to http://127.0.0.1:5000/"); app.run(host='0.0.0.0', port=5000, debug=False)